...
import { fetchAndAssignCustomerData } from './customer-data';
...
const Customers: React.FC<CustomersProps> = () => {
  ...
  const [accountCustomers, setAccountCustomers] = useState<AccountCustomers[]>([]);
  ...
  useEffect(() => {
    async function fetchData() {
      const data = await fetchAndAssignCustomerData();
      setAccountCustomers(data);
    }
    fetchData();
  }, []);
...
//   const handleView = async (record: AccountCustomers) => {
//     const { customer_code, customer_code } = record;
//     const otp = await fetchOTPFromServer(customer_code);
...
//   };
  ...
  const handleOtpVerification = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (enteredOTP === serverOTP && selectedAccountNumber !== null) {
      navigate(`/ui/tables/Customersresults/${selectedAccountNumber}`);
    } else {
  ...
  const columns: ReadonlyArray<Column> = [
    ...
    },
    {
      ...
      accessor: 'name', // Update accessor to 'name' field from the fetched data
      defaultCanSort: true,
    },
    {
      Header: 'Customer Email',
      accessor: 'email', // Update accessor to 'email' field from the fetched data
      defaultCanSort: true,
    },
    // {
    //   Header: 'Actions',
  ...
  const sizePerPageList: PageSize[] = [
    ...
    { text: '10', value: 10 },
    { text: '25', value: 25 },
    { text: 'All', value: accountCustomers.length },
  ];


  return (
    <>
      <PageBreadcrumb title="Customers" subName="Customers" />
      <Row>
        <Col>
          <Card>
            <Card.Header>
